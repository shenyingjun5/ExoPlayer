...
- 2026-07-10：新增 `RtpPacketReorderingQueueTest` 覆盖连续 `65534,65535,0,1`、跨 wrap 乱序、wrap 后迟到旧包。验证通过：targeted `:library-rtsp:testDebugUnitTest --tests com.google.android.exoplayer2.source.rtsp.RtpPacketReorderingQueueTest`、完整 `:library-rtsp:testDebugUnitTest`、`:library-rtsp:assembleRelease`。待发布为下一个 Maven artifact `2.19.1-labi.13`。
