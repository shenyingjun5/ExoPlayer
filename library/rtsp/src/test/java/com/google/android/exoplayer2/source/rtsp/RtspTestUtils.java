...
/* package */ final class RtspTestUtils {
  ...
  // RTP-Info urls are resolved against the session uri, so they must stay relative. An absolute
  // url with a hard-coded port (e.g. "rtsp://localhost/test/%s") never equals the track uri that
  // RtspMediaTrack derives from the Content-Base header, which is a different port.
  private static final String RTP_TIME_FORMAT = "url=%s;seq=%d;rtptime=%d";
